...
class Smarty_Resource_Email extends Smarty_Resource_Custom
{
    ...
    protected function fetch($name, &$source, &$mtime)
    {
        ...
        $row = DB::row("select `tpl_$tpl_field`,`last_update`
                  ...
                  where `tpl_name`=:tpl_name AND `lang`=:language", [':tpl_name'=>$tpl_name, ':language'=>lang::$language]);
        if (count($row) == 0) {
            $row = DB::row("select `tpl_$tpl_field`,`last_update`
                   from `email_templates`
                  where `tpl_name`=:tpl_name AND `lang`=:language", [':tpl_name'=>$tpl_name, ':language'=>lang::$default_lang]);
        }
